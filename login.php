...
//     // รับข้อมูล booking_id จากการร้องขอ
//     $booking_id = intval($request->getParsedBody()['booking_id']);
    
//     // ตรวจสอบว่ามีการอัปโหลดไฟล์หรือไม่
//     if (!isset($_FILES['payment_proof']) || $_FILES['payment_proof']['error'] !== UPLOAD_ERR_OK) {
//         $response->getBody()->write(json_encode(['error' => 'Payment proof file is required']));
//         return $response->withHeader('Content-Type', 'application/json')->withStatus(400);
//     }

//     // ตรวจสอบว่า booking_id ถูกส่งมาหรือไม่
//     if (empty($booking_id)) {
//         $response->getBody()->write(json_encode(['error' => 'Booking ID is required']));
//         return $response->withHeader('Content-Type', 'application/json')->withStatus(400);
//     }

//     // กำหนดโฟลเดอร์เก็บไฟล์ที่อัปโหลด
//     $uploadDir = 'C:/Users/Admin/OneDrive/เดสก์ท็อป/payment/';
//     if (!is_dir($uploadDir)) {
...
//     $fileName = 'payment_' . $booking_id . '_' . time() . '.' . pathinfo($uploadedFile['name'], PATHINFO_EXTENSION);
//     $filePath = $uploadDir . $fileName;

//     // ย้ายไฟล์ไปยังโฟลเดอร์เก็บไฟล์
//     if (!move_uploaded_file($uploadedFile['tmp_name'], $filePath)) {
//         $response->getBody()->write(json_encode(['error' => 'Failed to upload payment proof']));
//         return $response->withHeader('Content-Type', 'application/json')->withStatus(500);
//     }
...
//     $date_diff = $start_date->diff($booking_date)->days;
...
//         // อัปเดตสถานะการชำระเงินและเก็บชื่อไฟล์สลิปการชำระเงินในฐานข้อมูล
//         $stmtUpdatePaymentStatus = $conn->prepare(
//             "UPDATE bookings SET payment_status = 'ชำระแล้ว', payment_proof_url = ? WHERE booking_id = ?"
//         );
//         $stmtUpdatePaymentStatus->bind_param("si", $fileName, $booking_id);
...
////////////อัพโหลดรูป
$app->post('/uploadPaymentSlip', function (Request $request, Response $response, array $args) {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadFilePath)) {
            // ส่งกลับชื่อไฟล์
            return $response->withJson(['success' => true, 'fileName' => $fileName]);
        } else {
            return $response->withJson(['success' => false, 'message' => 'ไม่สามารถอัปโหลดไฟล์ได้'], 500);
        }
    }

    return $response->withJson(['success' => false, 'message' => 'ไม่มีไฟล์ที่อัปโหลด'], 400);
});
...
$app->get('/members23', function (Request $request, Response $response, array $args) use ($conn) {
    $sql = "SELECT first_name, last_name, phone, email FROM members WHERE member_status = 'moderator'";
